...
def parse_clinvar_tree(handle, dest=sys.stdout, multi=None, verbose=True, genome_build='GRCh37'):
    ...
    for event, elem in ET.iterparse(handle):
        ...
        list_sampledescription = [
            x.text for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/SampleDescription/Description') if x is not None 
        ] 
        current_row['sample_sampledescription'] = ";".join(list_sampledescription)
        print(" sampledescrition " + current_row['sample_sampledescription'])

        """
        numsmp = elem.find('.//ClinVarAssertion/ObservedIn/Sample/SampleDescription/Description') 
        current_row['sample_sampledescription']= ''
        if numsmp is not None and numsmp.text is not None:
            #print(" sample description " + numsmp.text)
            if current_row['sample_sampledescription'].strip():
               current_row['sample_sampledescription'] =  current_row['sample_sampledescription'] + ";" + numsmp.text
               print("sample description  add [" + current_row['sample_sampledescription'])
            else:
               current_row['sample_sampledescription'] = numsmp.text
               print("sample description assign [" + current_row['sample_sampledescription'])
        """
        ...
        current_row['observeddata_attribute'] = ";".join([ 
            x.text for x in elem.findall('.//ReferenceClinVarAssertion/ObservedIn/ObservedData/Attribute') if x is not None and x.text is not None
        ])
