...
HEADER = ['chrom', 'pos', 'ref', 'alt', 'variation_id', 'rcv', 
          'allele_id', 
          'clinical_significance_ordered', 'pathogenic', 
          'benign', 'review_status', 
          'all_submitters', 'all_traits',
          'all_pmids', 'inheritance_modes', 'age_of_onset', 'prevalence',
          'disease_mechanism',  'method_methodtype',
          'sample_affected_status',

          'sample_age','sample_ethnicity','sample_familydata_numfamilies',
          'sample_familydata_numfamilieswithsegregationobserved', 'sample_familydata_numfamilieswithvariant',
          'sample_familydata_segregationobserved', 'sample_sampledescription', 
          'sample_species', 'observeddata_attribute', 'sample_familydata_familyhistory',
...
def parse_clinvar_tree(handle, dest=sys.stdout, multi=None, verbose=True, genome_build='GRCh37'):
    ...
    for event, elem in ET.iterparse(handle):
        ...
        # now find the methodtype
        current_row['method_methodtype'] = ';'.join([ 
            x.text for x in elem.findall('.//ClinVarAssertion/ObservedIn/Method/MethodType') if x is not None
        ])

        
        # now find the sample_affected_status 
        current_row['sample_affected_status'] = ';'.join([
            x.text for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/AffectedStatus') if x is not None
        ])

        
        # now find the sample_age 
        current_row['sample_age'] = ';'.join([
            x.text for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/Age') if x is not None
        ])

        # now find the sample_ethnicity 
        current_row['sample_ethnicity'] = ';'.join([
            x.text for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/Ethnicity') if x is not None and x.text is not None
        ])

        # now find the sample_familydata_numfamilies 
        current_row['sample_familydata_numfamilies']= '' 
        numf = elem.find('./ClinVarAssertion/ObservedIn/Sample/FamilyData')
        if numf is not None and numf.attrib.get('NumFamilies') is not None:
            if current_row['sample_familydata_numfamilies'].strip():  
               current_row['sample_familydata_numfamilies'] =  current_row['sample_familydata_numfamilies'] + ";" + numf.attrib.get('NumFamilies')  
            else:
               current_row['sample_familydata_numfamilies'] = numf.attrib.get('NumFamilies') 



        numsegobs = elem.find('./ClinVarAssertion/ObservedIn/Sample/FamilyData')
        current_row['sample_familydata_numfamilieswithsegregationobserved']= ''
        if numsegobs is not None and numsegobs.attrib.get('NumFamiliesWithSegregationObserved') is not None:
            if current_row['sample_familydata_numfamilieswithsegregationobserved']:
               current_row['sample_familydata_numfamilieswithsegregationobserved'] = current_row['sample_familydata_numfamilieswithsegregationobserved'] +';'+ numsegobs.attrib.get('NumFamiliesWithSegregationObserved')
            else:
               current_row['sample_familydata_numfamilieswithsegregationobserved'] = numsegobs.attrib.get('NumFamiliesWithSegregationObserved')


        current_row['sample_familydata_numfamilieswithvariant']= '' 
        numv = elem.find('./ClinVarAssertion/ObservedIn/Sample/FamilyData')
        if numv is not None and numv.attrib.get('NumFamiliesWithVariant') is not None:
            if current_row['sample_familydata_numfamilieswithvariant'].strip():  
               current_row['sample_familydata_numfamilieswithvariant'] =  current_row['sample_familydata_numfamilieswithvariant'] + ";" + numv.attrib.get('NumFamiliesWithVariant')  
               print ("numv good " + current_row['sample_familydata_numfamilieswithvariant'] )
            else:
               current_row['sample_familydata_numfamilieswithvariant'] = numv.attrib.get('NumFamiliesWithVariant') 
               print ("numv bad " + current_row['sample_familydata_numfamilieswithvariant'] )

        numseg = elem.find('./ClinVarAssertion/ObservedIn/Sample/FamilyData')
        current_row['sample_familydata_segregationobserved']= ''
        if numseg is not None and numseg.attrib.get('SegregationObserved') is not None:
            if current_row['sample_familydata_segregationobserved'].strip():
               current_row['sample_familydata_segregationObserved'] = current_row['sample_familydata_segregationobserved'] +';'+ numseg.attrib.get('SegregationObserved')
            else:
               current_row['sample_familydata_segregationobserved'] = numseg.attrib.get('SegregationObserved')

        """
        current_row['sample_sampledescription']= ''
        numsmp = elem.find('.//ClinVarAssertion/ObservedIn/Sample/SampleDescription/Description') 
        if numsmp is not None and numsmp.text:
            current_row['sample_sampledescription'] = numsmp.text
            print("sample description not none 1 [" + current_row['sample_sampledescription'])
            print("sample description not none 2 [" + numsmp.text + "]")
        else:
            current_row['sample_sampledescription'] = '' 
            print("sample description none [" + current_row['sample_sampledescription'])
        """

        # now find the sample_description 
        current_row['sample_sampledescription'] = "||".join([
              x.text for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/SampleDescription/Description') if x is not None and x.text is not None 
        ])

        

        # now find the sample_species 
        current_row['sample_species']= ';'.join([
            x.text for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/Species') if x is not None
        ])
        ...
        # find the sample_familyhistory
        current_row['sample_familydata_familyhistory'] = "||".join([
            x.text for x in elem.findall('./ClinVarAssertion/ObservedIn/Sample/FamilyData/FamilyHistory') if x is not None and x.text is not None
        ])
        ...
        if (len(current_row['sample_sampledescription']) > 0):
           print ("HHH 1 " + current_row['sample_sampledescription'])
           print ("HHH 2 " + current_row['sample_sampledescription'])
           print (" len [" + str(len(measure)) + "]")


        for i in range(len(measure)):
            print ("HHH 11 " + current_row['sample_sampledescription'])
            ...
            print ("HHH 12 " + current_row['sample_sampledescription'])
            ...
            print ("HHH 13 " + current_row['sample_sampledescription'])
            ...
            print ("HHH 14 " + current_row['sample_sampledescription'])
            ...
            print ("HHH 15 " + current_row['sample_sampledescription'])
            ...
            print ("HHH 16 " + current_row['sample_sampledescription'])

            column_name = 'molecular_consequence'

            print ("HHH 17 " + current_row['sample_sampledescription'])
        ...
        elem.clear()
        print (" Again " + current_row['sample_sampledescription'])
