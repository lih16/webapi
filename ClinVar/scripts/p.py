...
def parse_clinvar_tree(handle, dest=sys.stdout, multi=None, verbose=True, genome_build='GRCh37'):
    ...
    for event, elem in ET.iterparse(handle):
        ...
        #print("sampledescrition [" + list_sampledescription + "]")
        print ('variant_id [' + current_row['variation_id'] + ']')
        for s in list_sampledescription:
           print 'sample,'.join(str(item) for item in s)
        ...
        if numsmp is not None and numsmp.text:
            ...
            if current_row['sample_sampledescription']:
