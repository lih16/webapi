...
HEADER = ['chrom', 'pos', 'ref', 'alt', 'start', 'stop', 'strand', 'variation_type', 'variation_id', 'rcv', 'scv',
          ...
          'disease_mechanism', 'origin', 'xrefs', 'dates_ordered', 'method_methodtype',
          ...
          'sample_familydata_segregationobserved', 
...
def parse_clinvar_tree(handle, dest=sys.stdout, multi=None, verbose=True, genome_build='GRCh37'):
    ...
    for event, elem in ET.iterparse(handle):
        ...
        affected_status = []
        one_submitter=''
        for temp_submitter in elem.findall('./ClinVarAssertion/ClinVarSubmissionID'):
            ...
            observedIn = elem.findall('.//ClinVarAssertion/ObservedIn')
            for j in range (len(observedIn)):
                
                sample_affected = observedIn[j].findall('.//Sample/AffectedStatus')
                if sample_affected[0].text is  not None:
                    
                    sample_methodtype = observedIn[j].findall('.//Method/MethodType')

                    if sample_methodtype[0].text is not None:
                        sample_methodtypevalue = sample_methodtype[0].text
                    else:
                        sample_methodtypevalue = 'NA' 

                    sample_age = observedIn[j].find('.//Sample/Age')
                    ...
                    if familydata is not None and familydata.attrib.get('NumFamilies') is not None:
                        family_NumFamilies = familydata[0].attrib.get('NumFamilies')
                    else:
                        family_NumFamilies = 'NA'
                    
                    if familydata is not None and familydata.attrib.get('NumFamiliesWithSegregationObserved') is not None:
                        family_NumFamiliesWithSegregationObserved = familydata[0].attrib.get('NumFamiliesWithSegregationObserved')
                    else:
                    ...
                    if familydata is not None and familydata.attrib.get('NumFamiliesWithVariant') is not None:
                        family_NumFamiliesWithVariant = familydata[0].attrib.get('NumFamiliesWithVariant')
                    else:
                        family_NumFamiliesWithVariant = 'NA' 

                    if familydata is not None and familydata.attrib.get('SegregationObserved') is not None:
                        family_SegregationObserved = familydata[0].attrib.get('SegregationObserved')
                    else:
                    ...
                    single_rec = one_submitter + " = Affected_Status:" + sample_affected[0].text + \
                                                     " SampleMethod:"+ sample_methodtypevalue +  \
                                                     " Sample_Age:" + sample_agevalue + \
                                                     " Sample_Ethnicity:" + sample_ethnicityvalue + \
                                                     " Family_NuMFamilies: " + family_NumFamilies + \
                                                     " Family_NuMFamiliesWithSegregationObserved: " + family_NumFamiliesWithSegregationObserved + \
                                                     " Family_NuMFamiliesWithVariant: " + family_NumFamiliesWithVariant + \
                                                     " Family_SegregationObserved: " + family_SegregationObserved + \
                                                     " Family_History: " + family_history
                                                      
          
                    affected_status.append(single_rec)
                else:
                    continue
                     
            
        current_row['sample_affected_status'] = ';'.join(affected_status)
        ...
        # now find the sample_familydata            
