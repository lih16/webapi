...
HEADER = ['chrom', 'pos', 'ref', 'alt', 'start', 'stop', 'strand', 'variation_type', 'variation_id', 'rcv', 'scv',
          ...
          'sample_observeddata_ordered','clinicalsignificance_comment_ordered', 
...
def parse_clinvar_tree(handle, dest=sys.stdout, multi=None, verbose=True, genome_build='GRCh37'):
    ...
    for event, elem in ET.iterparse(handle):
        ...
        for i in range (len(ClinVarAssertion)):
            ...
            comment = ClinVarAssertion[i].find('.//ClinicalSignificance/Comment')
            ...
            for j in range (len(observedIn)):
                ...
                if sample_affected.text is  not None:
                    ...
                    for k in range(len(sample_observeddatas)):
                       
                       if sample_observeddatas[k] is not None:
                           if sample_methodtypevalue == 'literature only':
                               citation_ids = sample_observeddatas[k].findall('.//Citation/ID')
                               for l in range(len(citation_ids)):
                                   if citation_ids[l] is not None:
                                       if l == 0:
                                           id = citation_ids[l].text
                                       else:
                                           id = id + ":" + citation_ids[l].text
                                   else:
                                       if l == 0:
                                           id = 'NA' 
                                       else:
                               ...
                               desc = sample_observeddatas[k].find('.//Attribute')
                               ...
                               desc = sample_observeddatas[k].find('.//Attribute')
                                   ...
                                   #print "000 obdata NOT none: " + desc.text + " submitter : " + one_submitter 
                               if desc.text is not None and desc.attrib.get('Type') == 'Description':
                                   #print "111 obdata NOT none: " + desc.text + " submitter : " + one_submitter + " type " + desc.attrib.get('Type') + " k " + str(k)
                                   desc_text = desc.text
                                   if k == 0:
                                        sample_observeddatavalue = "Description" + "->" + desc_text
                                        #print "222 sample_observedvalue" + sampl_observedd 
                                   else:
                           ...
                           sample_observeddatavalue = 'NA' 
                           #print "obdata none: " + " submitter : " + one_submitter 

                    if sample_methodtypevalue == 'literature only':
                        clinical_commentvalue = sample_observeddatavalue 
                    else: 
                        if comment is not None and comment.text is not None:
                             clinical_commentvalue = comment.text
                        else:
                             clinical_commentvalue = 'NA'
                    ...
                    if j > 0: 
                        ...
                        if sample_methodtypevalue == 'literature only':
                            clinical_comment_rec = sample_obdata_rec 
                        else:
                            clinical_comment_rec = header_rec + clinical_commentvalue
                        ...
                        if sample_methodtypevalue == 'literature only':
                            sample_obdata_rec = sample_observeddatavalue
                            clinical_comment_rec = sample_obdata_rec 
                        else:
            ...
            comment_arr.append(clinical_comment_rec)
        ...
        current_row['clinicalsignificance_comment_ordered'] = '||'.join(comment_arr)
        ...
        for traitset in elem.findall('.//ClinVarAssertion/ObservedIn/TraitSet/Trait'):
            if (traitset.attrib is not None and traitset.attrib.get('Type') == 'Finding'):
               ...
               if pheno_name_node is not None:
                   phenotype_ob_arr.append(pheno_name_node.text) 

        current_row['phenotype_trait_clinvarassertion_observedin'] = "||".join(phenotype_ob_arr) 
        ...
        for traitset in elem.findall('.//ClinVarAssertion/TraitSet/Trait'):
            if (traitset.attrib is not None and traitset.attrib.get('Type') == 'Disease'):
                pheno_name_node = traitset.find('.//Name/ElementValue')
                if pheno_name_node is not None:
                   phenotype_tr_arr.append(pheno_name_node.text)

        current_row['phenotype_trait_clinvarassertion_traitset'] = "||".join(phenotype_tr_arr) 
