...
HEADER = ['chrom', 'pos', 'ref', 'alt', 'start', 'stop', 'strand', 'variation_type', 'variation_id', 'rcv', 'scv',
      ...
      #    'disease_mechanism', 'origin', 'xrefs', 'dates_ordered', 'method_methodtype']
          ...
          'sample_familydata_numfamillieswithsegregationobserved', 'sample_familydata_numfamilieswithvariant',
          'sample_familydata_segregationobserved', 'sample_sampledescription', 
          'sample_species', 'observeddata_attri', 'sample_familydata_familyhistory',
...
def parse_clinvar_tree(handle, dest=sys.stdout, multi=None, verbose=True, genome_build='GRCh37'):
    ...
    for event, elem in ET.iterparse(handle):
        ...
        current_row['sample_familydata_numfamilies']= ';'.join([
            x.attrib.get('NumFamilies') for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/FamilyData/') if x is not None and x.attrib.get('NumFamilies') is not None
        ])

        # now find the sample_familydata_numfamilieswithsegregationobserved 
        current_row['sample_familydata_numfamillieswithsegregationobserved']= ';'.join([
            x.attrib.get('NumFamiliesWithSegregationObserved') for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/FamilyData/') if x is not None and x.attrib.get('NumFamiliesWithSegregationObserved') is not None
        ])

        # now find the sample_familydata_numfamilieswithvariant 
        current_row['sample_familydata_numfamilieswithvariant']= ';'.join([
            x.attrib.get('NumFamiliesWithVariant') for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/FamilyData/') if x is not None and x.attrib.get('NumFamiliesWithVariant') is not None
        ])

        # now find the sample_familydata_segregationobserved 
        current_row['sample_familydata_segregationobserved']= ';'.join([
            x.attrib.get('SegregationObserved') for x in elem.findall('.//ClinVarAssertion/ObservedIn/Sample/FamilyData/') if x is not None and x.attrib.get('SegregationObserved')
        ])


        # now find the sample_sampledescription 
        ...
        # now find the observeddata_attri 
        current_row['observeddata_attri'] = ";".join([ 
